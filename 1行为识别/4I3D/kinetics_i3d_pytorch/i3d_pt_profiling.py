...
# Launch `kernprof -lv i3d_pt_profiling.py --frame_nb 16`
...
def run(model, dataloader, criterion, optimizer, frame_nb):
    ...
    for i, (input_2d, target) in enumerate(dataloader):
        #input_2d.size()=(torch.Size([2, 3, 224, 224])
        optimizer.zero_grad()
        # Prepare data for pytorch forward pass
        input_3d = input_2d.clone().unsqueeze(2).repeat(1, 1, frame_nb, 1, 1) #[2, 3, 16, 224, 224]
